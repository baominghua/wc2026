...
$LocalScript = Join-Path $PSScriptRoot "diagnose-nas-firewall.sh"
$RemoteScript = "/tmp/wc2026-diagnose-firewall.sh"
$RemoteLog = "/tmp/wc2026-firewall-diag.log"
$LocalLog = Join-Path $env:TEMP ("wc2026-firewall-diag-{0}.log" -f (Get-Date -Format "yyyyMMdd-HHmmss"))

Write-Host "WC2026 NAS firewall diagnostics" -ForegroundColor Cyan
Write-Host "Target: $NasUser@$NasHost`:$NasSshPort" -ForegroundColor Cyan
Write-Host "This is read-only. You may be asked for the NAS password three times: upload, execute, download." -ForegroundColor Yellow
