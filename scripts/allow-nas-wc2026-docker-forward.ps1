param(
  [string]$NasHost = "bmhlfc.top",
  [string]$NasUser = "admin",
  [int]$NasSshPort = 22,
  [string]$NasPath = "/share/CACHEDEV1_DATA/Web/wc2026",
  [string]$LanCidr = "192.168.100.0/24"
)

$ErrorActionPreference = "Stop"

$LocalScript = Join-Path $PSScriptRoot "allow-nas-wc2026-docker-forward.sh"
$RemoteScript = "/tmp/wc2026-allow-docker-forward.sh"
$RemoteLog = "/tmp/wc2026-allow-docker-forward.log"
$LocalLog = Join-Path $env:TEMP ("wc2026-allow-docker-forward-{0}.log" -f (Get-Date -Format "yyyyMMdd-HHmmss"))

Write-Host "WC2026 Docker forward allow rule" -ForegroundColor Cyan
Write-Host "Target: $NasUser@$NasHost`:$NasSshPort" -ForegroundColor Cyan
Write-Host "Allow: $LanCidr -> wc2026-frontend:80 in Docker forward chains" -ForegroundColor Yellow
Write-Host "This is temporary and narrow; QNAP/Container Station may reset it later." -ForegroundColor Yellow
Write-Host "You may be asked for the NAS password three times: upload, execute, download." -ForegroundColor Yellow
Write-Host "Local log will be: $LocalLog" -ForegroundColor DarkGray

& scp -P $NasSshPort $LocalScript "${NasUser}@${NasHost}:$RemoteScript"
if ($LASTEXITCODE -ne 0) {
  throw "Upload failed with exit code $LASTEXITCODE."
}

& ssh -tt -p $NasSshPort "${NasUser}@${NasHost}" "sh '$RemoteScript' '$RemoteLog' '$NasPath' '$LanCidr'"
if ($LASTEXITCODE -ne 0) {
  throw "Remote allow failed with exit code $LASTEXITCODE."
}

& scp -P $NasSshPort "${NasUser}@${NasHost}:$RemoteLog" $LocalLog
if ($LASTEXITCODE -ne 0) {
  throw "Download failed with exit code $LASTEXITCODE."
}

Write-Host "Allow rule applied. Log saved to $LocalLog" -ForegroundColor Green
Get-Content $LocalLog
Read-Host "Press Enter to close"
